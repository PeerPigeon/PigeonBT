export type MeshNode = { id: string; label?: string; neighbors: Set<string>; lastSeen: number; online: boolean };
...
export class Topology {
  ...
  addNode(id: string, label?: string) {
    const n = this.nodes.get(id) || { id, label, neighbors: new Set<string>(), lastSeen: Date.now(), online: true };
...
export class MeshNetwork {
  ...
  readonly events = new EventBus<{ message: MeshMessage; state: { nodes: ReturnType<Topology['snapshot']>['nodes']; edges: ReturnType<Topology['snapshot']>['edges'] } }>();
  ...
  private bindAdapter() {
    this.adapter.on('deviceFound', ({ id, name }) => { this.topology.addNode(id, name); this.emitState(); });
